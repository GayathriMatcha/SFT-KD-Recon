...
def train_epoch(args, epoch,modelT,modelS,data_loader, optimizer, writer):
    ...
    for iter, data in enumerate(loop):
        ...
        # last layer 
        outputT_feat = [outputT[0][-1],outputT[1][-1],outputT[2][-1],outputT[3][-1]]
        outputS_feat = [outputS[0][-1],outputS[1][-1],outputS[2][-1],outputS[3][-1]]
        ...
        loss_group = kd(outputS_feat,outputT_feat)
                   
        loss   = reduce((lambda x,y : x + y),loss_group)  / len(loss_group)
...
#             outputT_feat  = outputT[2][-1]
#             outputS_feat  = outputS[2][-1]
#             print(outputT_feat.dtype)

        # last layer 
            outputT_feat = [outputT[0][-1],outputT[1][-1],outputT[2][-1],outputT[3][-1]]
            outputS_feat = [outputS[0][-1],outputS[1][-1],outputS[2][-1],outputS[3][-1]]
            
#             outputT_feat = [outputT[0][-1],outputT[3][-1]]
#             outputS_feat = [outputS[0][-1],outputS[3][-1]]
...
#             s_fsp = compute_fsp(outputS_feat)
#             t_fsp = compute_fsp(outputT_feat)
#             loss_group = [compute_loss(s, t) for s, t in zip(s_fsp, t_fsp)]
            loss_group = kd(outputS_feat,outputT_feat)        
            loss   = reduce((lambda x,y : x + y),loss_group)  / len(loss_group)
